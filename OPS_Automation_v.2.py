...
def date_checker_st_dt(input_st_dt):
    try: 
        global input_dt_ot
        input_dt_ot=input_st_dt
        global check_dt
        check_dt=dt.strptime(input_st_dt,"%d-%m-%Y")
        global output_st_dt
        global check_wk_dt
        output_st_dt=check_dt.strftime("%Y-%m-%d")
        check_wk_dt=check_dt.weekday() in [4] # if is weekend or not
        if check_wk_dt==False:
            print('\033[31;1mInput date is not weekend date\033[0m')
            new_input_dt=input('Enter Weekending Date (DD-MM-YYYY): ')
            date_checker_st_dt(new_input_dt)
        elif input_st_dt not in ms_2070_dt:
            print('\033[31;1mInput date is not present in PRIMA\033[0m')
            new_input_dt=input('Enter Weekending Date (DD-MM-YYYY): ')
            date_checker_st_dt(new_input_dt)
    except ValueError or NameError:
        print('\033[31;1mInput date is Invalid date format\033[0m')
...
def date_checker_end_dt(input_end_dt):
    try: 
        global input_dt_ot
        input_dt_ot=input_end_dt
        global check_dt
        check_dt=dt.strptime(input_end_dt,"%d-%m-%Y")
        global output_end_dt
        global check_wk_dt
        output_end_dt=check_dt.strftime("%Y-%m-%d")
        check_wk_dt=check_dt.weekday() in [4] # if is weekend or not
        if check_wk_dt==False:
            print('\033[31;1mInput date is not weekend date\033[0m')
            new_input_dt=input('Enter Weekending Date (DD-MM-YYYY): ')
            date_checker_end_dt(new_input_dt)
        elif input_end_dt not in ms_2070_dt:
            print('\033[31;1mInput date is not present in PRIMA\033[0m')
            new_input_dt=input('Enter Weekending Date (DD-MM-YYYY): ')
            date_checker_st_dt(new_input_dt)
    except ValueError or NameError:
...
class PRIMA_MS():
    def __init__(self, p_emp_id, p_account_id, p_work_item,p_weekend_date,p_activity_code,p_total_hours,emp_lastname):
...
def prima_ms():
    ...
    for row in range(5,wbs12.max_row+1):
        ...
        if emp_id:
            ...
            emp_lastname=wbs12.cell(row=row, column=23).value.strip()
            match_emp=None
            ...
            if match_emp:
                ...
                emp_prima_data_ms.append(PRIMA_MS(emp_id,p_account_id, p_work_item,new_p_weekend_date,new_p_activity_code,new_p_total_hours,emp_lastname))
                new_dt=dt.strptime(new_p_weekend_date,"%Y-%m-%d")
                new_dt_ft=new_dt.strftime("%d-%m-%Y")
                if new_dt_ft not in ms_2070_dt:
                    ms_2070_dt.append(new_dt_ft)
...
def report_2070(LOB):
    wb31=openpyxl.Workbook() # Create a new workbook
    wbs31=wb31.active
    wbs31.title='PRIMA'
    wbs31.append(["EMP ID",'Emp Lastname','Hours'])
    if LOB=='RS MS':
        hrs=45
    elif LOB=='Empower':
        hrs=45
    ...
    emp_details={}
    for prima_data in emp_prima_data_ms:
        if prima_data.p_emp_id in emp_details:
            # print(output_st_dt,prima_data.p_weekend_date,output_end_dt)
            if output_st_dt<=prima_data.p_weekend_date<=output_end_dt:
                emp_details[prima_data.p_emp_id][1]+=int(prima_data.p_total_hours)
                emp_details[prima_data.p_emp_id][2]+=1
        elif prima_data.p_emp_id not in emp_details:
            if output_st_dt<=prima_data.p_weekend_date<=output_end_dt:
                emp_details[prima_data.p_emp_id]=[prima_data.emp_lastname,int(prima_data.p_total_hours),1]
    #for empid,hrs in emp_details.items():
     #   print(empid,hrs)
    #output_st_dt,output_end_dt
    counter1=2
    for empid,emp_dt in emp_details.items():
        wbs31[f'A{counter1}']=empid
        wbs31[f'B{counter1}']=emp_dt[0]
        wbs31[f'C{counter1}']=emp_dt[1]
        counter1+=1
    wb31.create_sheet(LOB+' Analysis')
    wbs32=wb31[LOB+' Analysis']
    wbs32.sheet_properties.tabColor= "36A832"       
    wbs32.append(["EMP ID",'Name','Team','Start Date','Billable / Bench','Chargeable','Chargeable UTE','Available','Remarks'])
    counter1=2
    for emp in emp_data:
        for empid,emp_dt in emp_details.items():
            if emp.emp_id==empid:
                wbs32[f'A{counter1}']=emp.emp_id
                wbs32[f'B{counter1}']=emp.name
                wbs32[f'C{counter1}']=emp.PIR_location
                wbs32[f'D{counter1}']=date_formatting(emp.bill_start_dt)
                wbs32[f'E{counter1}']=emp.billable_bench
                wbs32[f'F{counter1}']=emp_dt[1]
                wbs32[f'H{counter1}']=f'={hrs}*{emp_dt[2]}'
                wbs32[f'G{counter1}']=f'=F{counter1}/H{counter1}'
                wbs32[f'G{counter1}'].number_format = FORMAT_PERCENTAGE
                wbs32[f'I{counter1}']=f'=IF(G{counter1}>=100%, "Above Target", "Below Target")'
                counter1+=1
    ...
    for cell in wbs32[1]:
        cell.fill=header_fill
        cell.font=header_font
    for row in range(wbs31.min_row, wbs31.max_row+1):
        for col in range(wbs31.min_column, wbs31.max_column+1):
            wbs31.cell(row=row,column=col).border=border
    for row in range(wbs32.min_row, wbs32.max_row+1):
        for col in range(wbs32.min_column, wbs32.max_column+1):
            wbs32.cell(row=row,column=col).border=border
    def report_save_2070():
        try:
            wb31.save(app_path+f'/{LOB}_2070_Report.xlsx')
        except PermissionError:
            print(f'\033[31;1m{LOB}_2070_Report.xlsx is opened. Please close the report before running the code. \033[0m')
...
if input_file_prima_ms=='skip' or input_file_prima_ms=='':
    ...
    def input_check():
        global ms_2070_dt
        input_file_2070_ms=input('>>> Please Enter the 2070 RS-MS reqiured (Y/N): ')
        if input_file_2070_ms=='N' or input_file_2070_ms=='n':      
            print('\033[34;1mRS-MS 2070 Report is skipped\033[0m')
        elif input_file_2070_ms=='Y' or input_file_2070_ms=='y':
            print("Available date in PRIMA MS: \n"+"\n".join(ms_2070_dt))

            st_dt=input('>>> Please Enter RS MS Start Date for 2070 (DD-MM-YYYY): ') 
            date_checker_st_dt(st_dt)
            end_dt=input('>>> Please Enter RS MS End Date for 2070 (DD-MM-YYYY): ')   
            date_checker_end_dt(end_dt)
...
if input_file_prima_emp=='skip' or input_file_prima_emp=='':
    ...
    def input_check():
        global ms_2070_dt
        input_file_2070_emp=input('>>> Please Enter the 2070 RS-MS reqiured (Y/N): ')
        if input_file_2070_emp=='N' or input_file_2070_emp=='n':      
            print('\033[34;1mEmpower 2070 Report is skipped\033[0m')
        elif input_file_2070_emp=='Y' or input_file_2070_emp=='y':
            print("Available date in PRIMA Empower: \n"+"\n".join(ms_2070_dt))
            st_dt=input('>>> Please Enter Empower Start Date for 2070 (DD-MM-YYYY): ') 
            date_checker_st_dt(st_dt)
            end_dt=input('>>> Please Enter Empower End Date for 2070 (DD-MM-YYYY): ')   
            date_checker_end_dt(end_dt)
...
if input_file_prima_grod=='skip' or input_file_prima_grod=='':
    ...
    def input_check():
        global ms_2070_dt
        input_file_2070_grod=input('>>> Please Enter the 2070 RS-MS reqiured (Y/N): ')
        if input_file_2070_grod=='N' or input_file_2070_grod=='n':      
            print('\033[34;1mGROD 2070 Report is skipped\033[0m')
        elif input_file_2070_grod=='Y' or input_file_2070_grod=='y':
            print("Available date in PRIMA GROD:")
            st_dt=input('>>> Please Enter GROD Start Date for 2070 (DD-MM-YYYY): ') 
            date_checker_st_dt(st_dt)
            end_dt=input('>>> Please Enter GROD End Date for 2070 (DD-MM-YYYY): ')   
            date_checker_end_dt(end_dt)
...
def delay_print(s):
    for c in s:
        ...
        sys.stdout.flush()
        time.sleep(0.10)
delay_print('\n\n\033[32;1;1m>>> Successfully Generated Report!! ;)\n\n>>> Thank you!! \033[0m\n\n\n')
